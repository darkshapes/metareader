...
class ReadModelTags:
    ...
    def read_metadata_from(self, file_path_named: str, separate_desc: bool = True) -> Dict[str, str]:
        ...
        if not any(extension in ext_type for ext_type in ExtensionType.MODEL if extension):
            ...
            if metadata is None:
                print(f"Couldn't load model metadata for {file_path_named}")
                return None
            return metadata
    ...
    def create_gguf_reader(self, file_path_named: str) -> dict:
        ...
        try:
            ...
            print("'gguf' llibrary not available")
        ...
        try:  # method using gguf library, better for LDM conversions
            ...
            print("Value error assembling GGUFReader >:V %s", error_log, tb=error_log.__traceback__)
        else:
            reader_data = None
        ...
        try:
            ...
            if parser:
                ...
                try:
                    ...
                    print("Parsing file failed for %s", file_path_named, error_log, tb=error_log.__traceback__)
        ...
        try:
            ...
            if separate_desc:  # in this case we leave the rest of the dict behind
                ...
                if isinstance(meta_copy.get(meta_key), dict):
                    try:
                        metadata = {
                            meta_key: {"dtype": tensor.dtype, "shape": tensor.size, "data_offsets": tensor.storage_offset} for meta_key, tensor in meta_copy.get(meta_key).items()
                        }
    ...
    def metadata_from_safetensors(self, file_path_named: str, separate_desc: bool = True) -> dict:
        ...
        with open(file_path_named, "rb") as file_contents_to:
            try:
                ...
                print("Failed to read json from file : %s", file_path_named, error_log, tb=error_log.__traceback__)
            else:
                if separate_desc:
                    try:
                        ...
                        print("Couldnt remove '__metadata__' from header data. %s", header_data, error_log, tb=error_log.__traceback__)
                return header_data
